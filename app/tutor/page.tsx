...
const firebaseConfig = {
  apiKey: "AIzaSyBiAD2tuonJbMBbyfCgD7tA1ya6S4paIrI",
  authDomain: "study-931be.firebaseapp.com",
  projectId: "study-931be",
  storageBucket: "study-931be.firebasestorage.app",
  messagingSenderId: "362246173631",
  appId: "1:362246173631:web:a40b447d74bb9fc6c1a70a"
};
...
export default function TutorApp() {
  ...
  const handlePublishQuestion = async (e: React.FormEvent) => {
    ...
    try {
      if (editingQuestionId) {
        ...
        setPublishMessage('問題を修正・更新しました！');
      } else {
        // 新規作成処理
  ...
  // --- 詳細トグル ---
  const toggleDetails = (qId: string) => {
      ...
      <header className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-md">
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg text-xs font-bold transition active:scale-95 ${
          ...
          <div className="flex items-center gap-1.5">
            ...
            <h1 className="font-bold text-base md:text-lg">チューター管理</h1>
          </div>
        </div>
          ...
          <div className="relative bg-slate-800 text-white w-64 max-w-sm h-full shadow-2xl ml-auto flex flex-col p-6 z-50">
            ...
            <nav className="space-y-3">
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${activeTab === 'create' ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
              >
                <span>📝</span><span>{editingQuestionId ? '問題の編集' : '問題作成'}</span>
              </button>

              <button
                onClick={() => { setActiveTab('answers'); setIsMenuOpen(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${activeTab === 'answers' ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${activeTab === 'records' ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}
              >
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-xl font-bold text-slate-800">
                      {editingQuestionId ? '✏️ 問題の修正・編集' : '新しい四択問題を作成'}
                    </h2>
                    <p className="text-xs text-slate-500 mt-1">
                      {editingQuestionId ? '問題の内容を修正して更新します。' : '作成した問題は即時に生徒側のアプリに配信されます。'}
                    </p>
                  </div>
                  {editingQuestionId && (
                    <button
                ...
                <form onSubmit={handlePublishQuestion} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">
                      四択の回答 ＆ 正解ラジオボタン選択 <span className="text-rose-500">*</span>
                    </label>
                      ...
                      {choices.map((choice, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <input
                            ...
                            id={`choice-${idx}`}
                          />
                          <label htmlFor={`choice-${idx}`} className="text-xs font-bold text-slate-500 w-6">
                            {idx + 1}.
                          </label>
                          <input
                            type="text"
                            ...
                            className="flex-1 p-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                            required
                          />
                          {correctAnswer === idx && (
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">
                      解説 <span className="text-xs font-normal text-slate-400">（任意）</span>
                    </label>
                    <textarea
                      rows={2}
                      value={explanation}
                      onChange={(e) => setExplanation(e.target.value)}
                      placeholder="正解の補足や解説を入力できます。"
                      className="w-full p-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm resize-none"
                    />
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition shadow text-sm disabled:opacity-50"
                    >
                      {isPublishing
                        ? '保存中...'
                        : editingQuestionId
                          ...
                          <div key={q.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                            <div className="flex justify-between items-center text-xs">
                              <span className="font-bold text-slate-700">
                                {student ? student.name : '不明な生徒'} さんからの質問
                              </span>
                              <span
                          ...
                          <div key={q.id} className="p-4 border border-slate-200 rounded-xl space-y-3">
                            <div className="flex justify-between items-start gap-2">
                              ...
                              <button
                                ...
                                className="text-xs px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 font-bold rounded-lg transition whitespace-nowrap"
                              >
                                ✏️ 修正
                              </button>
                            </div>
                            ...
                            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                              <div className="flex gap-4">
                                <span>
                                  回答数: <strong className="text-slate-800">{answersForQ.length}</strong> 人
                                </span>
                                <span>
                                  正答率:{' '}
                                  <strong className="text-emerald-600">
                                    {answersForQ.length > 0
                                      ? Math.round((correctCount / answersForQ.length) * 100)
                                      : 0}
                                    %
                                  </strong>
                                </span>
                              ...
                              {answersForQ.length > 0 && (
                                ...
                                >
                                  {isExpanded ? '詳細を隠す ▲' : '回答者の詳細を見る ▼'}
                                </button>
                              )}
                            </div>

                            {isExpanded && answersForQ.length > 0 && (
                              <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
                                <ul className="space-y-1.5">
                                  {answersForQ.map((ans) => {
                                    const student = users.find((u) => u.uid === ans.userId);
                                    return (
                                      <li key={ans.id} className="flex justify-between items-center text-xs bg-slate-50 border border-slate-100 p-2.5 rounded-lg">
                                        <span className="font-bold text-slate-700">
                                          {student ? student.name : '不明な生徒'}
                                        </span>
                                        <div className="flex items-center gap-3">
                                          <span className="text-slate-500">
                                            選択した答え: <span className="font-bold text-slate-700">{ans.selectedChoice + 1}番</span>
                                          </span>
                                          {ans.isCorrect ? (
                                            <span className="text-emerald-700 font-bold bg-emerald-100 px-2 py-0.5 rounded">正解</span>
                                          ) : (
                                            <span className="text-rose-700 font-bold bg-rose-100 px-2 py-0.5 rounded">不正解</span>
                                          )}
                                        </div>
                                      </li>
                                    );
            ...
            {activeTab === 'records' && (
              <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    生徒側の「記録」画面と同じデータがリアルタイムで共有されています。
                  </p>
                </div>
                      ...
                      >
                        ...
                        <div className="grid grid-cols-7 gap-1 pt-1">
                          {rec.days.map((day, dIdx) => (
                            <div
                              key={dIdx}
                              className={`p-2 rounded-lg text-center border text-xs space-y-1 ${
                                day.isLoggedIn
                            ...
                            >
                              ...
                              <div className="text-[9px] font-semibold text-slate-600">
                                {day.solvedCount > 0 ? `${day.solvedCount}問` : ''}
                              </div>
                            </div>
                          ))}
                        </div>
