...
export default function StudentApp() {
  ...
  const loadRecordsData = async () => {
    ...
    try {
      ...
      if (userDoc.exists() && userDoc.data().lastLoginDate) {
        lastLoginKey = (userDoc.data().lastLoginDate as Timestamp)
          .toDate()
          .toISOString()
          .split('T')[0];
      }
  ...
  const handleNavigate = (
    tab: 'home' | 'questions' | 'review' | 'records' | 'settings'
  ) => {
    setActiveTab(tab);
    setIsMenuOpen(false);
        ...
        <div className="fixed inset-0 z-40 flex">
          ...
          <div className="relative bg-white w-64 max-w-sm h-full shadow-2xl ml-auto flex flex-col p-6 z-50">
            ...
            <nav className="space-y-2">
              <button
                onClick={() => handleNavigate('home')}
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${
                  activeTab === 'questions' ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                <span>✏️</span>
                <span>チューターからの問題</span>
                {unansweredCount > 0 && (
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${
                  activeTab === 'review' ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                <span>🔄</span>
                <span>復習・質問</span>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${
                  activeTab === 'records' ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                <span>📅</span>
                <span>学習記録</span>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 font-medium rounded-xl transition ${
                  activeTab === 'settings' ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                <span>⚙️</span>
                <span>設定</span>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl text-center space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 py-2 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-medium transition"
              >
                後で
              </button>
              <button
                ...
                className="flex-1 py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow transition"
              >
                問題を解く
              </button>
            </div>
        ...
        {activeTab === 'home' && (
          <div className="space-y-6">
            ...
            <section className="grid grid-cols-2 gap-4">
              <button
                onClick={() => handleNavigate('questions')}
                className="p-5 bg-white hover:bg-blue-50/50 border border-slate-200 rounded-2xl text-left transition space-y-2 shadow-sm relative"
              >
              ...
              <button
                onClick={() => handleNavigate('review')}
                className="p-5 bg-white hover:bg-amber-50/50 border border-slate-200 rounded-2xl text-left transition space-y-2 shadow-sm"
              >
              ...
              <button
                onClick={() => handleNavigate('records')}
                className="p-5 bg-white hover:bg-emerald-50/50 border border-slate-200 rounded-2xl text-left transition space-y-2 shadow-sm"
              >
                <div className="text-3xl">📅</div>
              ...
              <button
                onClick={() => handleNavigate('settings')}
                className="p-5 bg-white hover:bg-purple-50/50 border border-slate-200 rounded-2xl text-left transition space-y-2 shadow-sm"
              >
                <div className="text-3xl">⚙️</div>
            ...
            {unansweredQuestions.length === 0 ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-center space-y-4">
                ...
                <p className="text-xs text-slate-500">
                  チューターが新しい問題を投稿するのを待ちましょう。
                </p>
                <button
                  onClick={() => handleNavigate('home')}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl transition shadow text-xs"
                >
                  ホームに戻る
                </button>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 min-h-[120px] flex items-center justify-center">
                  <p className="text-base font-bold text-slate-800 text-center">
                    {unansweredQuestions[currentQIndex].content}
                  </p>
                </div>
                ...
                <div className="space-y-2.5">
                  {unansweredQuestions[currentQIndex].choices.map((choice, idx) => {
                    let btnStyle =
                      'w-full p-4 border rounded-xl text-left text-sm font-medium transition text-slate-700 border-slate-200 hover:border-blue-300';

                    if (selectedChoice === idx) {
                      btnStyle =
                        'w-full p-4 border-2 rounded-xl text-left text-sm font-medium transition border-blue-600 bg-blue-50 text-blue-700';
                    }

                    if (isQAnswered) {
                      if (idx === unansweredQuestions[currentQIndex].correctAnswer) {
                        btnStyle =
                          'w-full p-4 border-2 rounded-xl text-left text-sm font-medium bg-emerald-100 border-emerald-500 text-emerald-800';
                      } else if (selectedChoice === idx) {
                        btnStyle =
                          'w-full p-4 border-2 rounded-xl text-left text-sm font-medium bg-rose-100 border-rose-500 text-rose-800';
                      }
                    }
                ...
                <div className="pt-2">
                  {!isQAnswered ? (
                    <button
                      ...
                      className={`w-full py-3 rounded-xl font-bold transition shadow text-xs ${
                        selectedChoice !== null
                    ...
                    <button
                      ...
                      className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition shadow text-xs"
                    >
                      {currentQIndex + 1 < unansweredQuestions.length ? '次の問題へ' : '結果を見る'}
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="flex bg-slate-200 p-1 rounded-xl">
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${
                  reviewTab === 'review' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600'
                }`}
              >
                間違えた問題 ({wrongQuestions.length})
              </button>
              <button
                onClick={() => setReviewTab('ask')}
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${
                  reviewTab === 'ask' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600'
                }`}
              >
                チューターに質問
              </button>
                        ...
                        {q.choices.map((choice, cIdx) => {
                          ...
                          return (
                            <button
                              key={cIdx}
                              disabled={isAnswered}
                              onClick={() => handleReviewSelect(q.id, cIdx, q.correctAnswer)}
                              className={style}
                            >
                              {cIdx + 1}. {choice}
                            </button>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
              <h2 className="font-bold text-slate-800 text-sm flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-400 font-normal">チューター共有中 🤝</span>
              </h2>
                ...
                {weekRecords.map((r, i) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl border transition ${
                      r.isLoggedIn ? 'bg-blue-50/40 border-blue-100' : 'bg-slate-50 border-slate-100'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs ${
                          r.isLoggedIn ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'
                        }`}
                      >
                        {r.isLoggedIn ? '✓' : '-'}
                      </div>
                    ...
                    <div>
                      {r.solvedCount > 0 ? (
                        <span className="inline-block bg-blue-100 text-blue-700 text-[10px] font-bold px-2.5 py-1 rounded-lg">
                          {r.solvedCount} 問解答
                        </span>
                      ) : (
                        <span className="text-[10px] text-slate-400">解答なし</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            ...
            <form onSubmit={handleSaveSettings} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
              ...
              {settingsStatus && (
                <p className="text-xs font-bold text-center text-emerald-600">{settingsStatus}</p>
              )}
            </form>
