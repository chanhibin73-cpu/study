...
export default function StudentApp() {
  ...
  const [myQuestionsHistory, setMyQuestionsHistory] = useState<StudentQuestion[]>([]);
  ...
  const loadReviewData = async () => {
    ...
    try {
      ...
      // 過去の質問履歴と返信の読み込み
      await loadMyQuestionsHistory(user.uid);
    } catch (error) {
      console.error('復習データ取得エラー:', error);
    }
  };

  // 自分の質問とチューター返信を取得する関数
  const loadMyQuestionsHistory = async (uid: string) => {
    try {
      const qQuery = query(
        collection(db, 'questionsToTutor'),
        where('userId', '==', uid)
      );
      const qSnap = await getDocs(qQuery);
      const history: StudentQuestion[] = qSnap.docs.map((d) => ({
        id: d.id,
        ...(d.data() as Omit<StudentQuestion, 'id'>),
      }));
      setMyQuestionsHistory(history);
    } catch (error) {
      console.error('質問履歴取得エラー:', error);
    }
  };
                  ...
                  wrongQuestions.map((q, idx) => {
                    ...
                    <div key={q.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        {dateStr && <span className="text-[10px] text-slate-400">出題: {dateStr}</span>}
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                  <h2 className="font-bold text-slate-800 text-sm flex items-center justify-between">
                    <span>💬 質問の履歴と返信</span>
                    <span className="text-xs text-slate-400 font-normal">{myQuestionsHistory.length} 件</span>
                  </h2>

                  {myQuestionsHistory.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-4">まだ質問の履歴はありません。</p>
                  ) : (
                    <div className="space-y-4">
                      {myQuestionsHistory.map((item) => (
                        <div key={item.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                          <div className="flex justify-between items-center text-xs">
                            <span className="font-bold text-slate-700">あなたの質問</span>
                            <span className={`font-bold px-2 py-0.5 rounded text-[10px] ${item.reply ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                              {item.reply ? '返信あり' : '回答待ち'}
                            </span>
                          </div>

                          <p className="text-xs text-slate-800 bg-white p-3 rounded-lg border border-slate-100">
                            {item.message}
                          </p>
                          ...
                          {item.reply ? (
                            <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-xs space-y-1">
                              <div className="flex items-center gap-1 text-indigo-700 font-bold">
                                <span>👨‍🏫</span>
                                <span>チューターからの返信:</span>
                              </div>
                              <p className="text-indigo-950 font-medium pl-5">{item.reply}</p>
                            </div>
                          ) : (
                            <p className="text-[10px] text-slate-400 italic">※ チューターからの返信をお待ちください。</p>
                          )}
                        </div>
                      ))}
                    </div>
