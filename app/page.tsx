...
export default function StudentApp() {
  ...
  const handleRegisterName = async (e: React.FormEvent) => {
    ...
    // ★ ここを修正: userがない場合は無言でストップせず、原因をアラートで表示します
    if (!user) {
      alert('認証エラー：\nFirebaseコンソールの「Authentication」で「匿名（Anonymous）」ログインが有効になっていない可能性があります。設定を確認してください。');
      return;
    }
    ...
    try {
      ...
      alert('エラーが発生しました！\nFirebaseのFirestoreデータベースが未作成か、セキュリティルールが設定されていない可能性があります。');
    }
  };
