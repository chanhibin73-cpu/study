...
export default function StudentApp() {
  ...
  // 匿名ログイン監視
  useEffect(() => {
    // 【改善】Vercelの環境変数が読み込めているかチェック
    if (!process.env.NEXT_PUBLIC_FIREBASE_API_KEY) {
      alert('【重要】Vercelの環境変数が反映されていません！\nVercelのSettingsで環境変数を入力した後、必ず「Redeploy（再デプロイ）」を実行してください。');
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        try {
          ...
          alert(`認証エラー: ${error.message}\nFirebaseのAuthenticationで「匿名」が有効になっているか確認してください。`);
          setNeedsNameRegistration(true);
          setLoading(false);
        }
      }
